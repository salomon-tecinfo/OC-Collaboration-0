...
namespace Orchard.CRM.Core.Activities
{
    public abstract class CRMSendEmailActivity : Task
    {
        ...
        protected void SendEmail(string subject, string body, string recipients, bool queued, int priority)
        {
            ...
            if (!queued)
            {
                this.messageService.Send(OrchardCollaborationSmtpMessageChannel.MessageType, parameters);
            }
            else
            {
                this.jobsQueueService.Enqueue("IMessageService.Send", new { type = OrchardCollaborationSmtpMessageChannel.MessageType, parameters = parameters }, priority);
            }
